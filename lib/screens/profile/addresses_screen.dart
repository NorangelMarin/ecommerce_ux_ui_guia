...
class AddressesScreen extends ConsumerWidget {
              ...
              data: (addresses) {
                if (addresses.isEmpty) {
                  return Padding(
                    ...
                    child: Text(
                      'no_tienes_direcciones_guardadas'.tr(),
  ...
  Widget _buildAddNewCard(BuildContext context, WidgetRef ref) {
    ...
    return GuideWrapper(
      ...
      description:
          'El uso de un diseño diferenciado (borde resaltado) y un icono "+" claro sirve como un "call to action" evidente que invita al usuario a interactuar intuitivamente.',
  ...
  Widget _buildGpsCard(BuildContext context, WidgetRef ref) {
    final theme = Theme.of(context);
    final currentAddressText = 'ubicacion_detectada_por_gps'.tr();

    return GuideWrapper(
      title: 'prevención_de_errores_y_eficiencia'.tr(),
      description:
          'Aprovechar el hardware del dispositivo (GPS) para geolocalizar al usuario previene errores tipográficos comunes al escribir direcciones manualmente y agiliza considerablemente el ingreso de datos.',
      child: Container(
        decoration: BoxDecoration(
          color: AppColors.of(context).blanco,
          borderRadius: BorderRadius.circular(12),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              Text(
                'agiliza_el_proceso_de_seleccin_de_ubicac'.tr(),
                style: theme.textTheme.bodyMedium?.copyWith(
                  color: AppColors.of(context).sombras,
                  fontSize: 12,
                  height: 1.4,
  ...
  Future<void> _handleGpsLocation(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      String addressText = 'ubicación_obtenida'.tr();
      String estado = '';
      String ciudad = '';
      String municipio = '';
      String urbanizacion = '';
              ...
              onPressed: () async {
                ...
                if (user != null) {
                  final newAddress = Address(
                    ...
                    label: alias,
                    labelColor: 'orange',
                    estado: estado.isNotEmpty ? estado : 'no_especificado'.tr(),
                    ciudad: ciudad.isNotEmpty ? ciudad : 'no_especificado'.tr(),
                    municipio: municipio.isNotEmpty
                        ? municipio
                        : 'no_especificado'.tr(),
                    urbanizacion: urbanizacion.isNotEmpty
                        ? urbanizacion
                        : 'no_especificado'.tr(),
                  );
                  await ref
  ...
  Color _getBadgeColor(BuildContext context, String colorKey) {
    switch (colorKey) {
      case 'orange':
